...
Vue.component('pub-sec', {
    props: ['image', 'id', 'type', 'authors', 'areas', 'venue', 'arxiv', 'pdf', 'link', 'linkname', 'poster', 'bibtex', 'code', 'demo', 'video', 'slides', 'abstract', 'cite', 'citelink'],
    template: function () {
        if (this.type == 'J') { J++; this.count = J; }
        if (this.type == 'C') { C++; this.count = C; }
        if (this.type == 'E') { E++; this.count = E; }
        if (this.type == 'P') { P++; this.count = P; }
        pubids[this.id] = this.type + this.count;
        console.log(this.count)
        return `<li class="li-pubs" id="{{id}}">
        <div class="row">
            <div class="imgcolumn">
                <img class="rounded-corner" style="width:100%;border-radius:10%;border:2px solid dimgrey;" src="./img/publications/{{image}}" alt="{{id}}" >
            </div>
            <div class="textcolumn">
                [{{type}}{{count}}]&nbsp;<b class="text-primary"> {{title}} </b>
                <br />
                ${`{{authors}}`.replace(/Baihan Lin/gi, '<b><span style="color: dimgray;">Baihan Lin</span></b>')}
                <br />
                <p> ${`{{areas}}`.split('-').map((item) => { return getFields(item, 1, 1) }).join('&nbsp;')} </p>
                <p style="color:black"> {{venue}} </p>
                ${`{{arxiv}}` ? `[ <a href="{{areas}}">arXiv</a> ]&nbsp;` : ``}
                ${`{{pdf}}` ? `[ <a href="./pdfs/{{pdf}}">pdf</a> ]&nbsp;` : ``}
                ${`{{link}}` ? `[  <a href="{{link}}">${`{{linkname}}` ? "{{linkname}}" : 'link'}</a> ]&nbsp;` : ``}
                ${`{{poster}}` ? `[ <a href="./pdfs/{{poster}}">poster</a> ]&nbsp;` : ``}
                ${`{{bibtex}}` ? `[ <a href="./bibTex/{{bibtex}}">bibTex</a> ]&nbsp;` : ``}
                ${`{{code}}` ? `[  <a href="{{code}}">code</a> ]&nbsp;` : ``}
                ${`{{demo}}` ? `[  <a href="{{demo}}">demo</a> ]&nbsp;` : ``}
                ${`{{video}}` ? `[  <a href="{{video}}">video</a> ]&nbsp;` : ``}
                ${`{{slides}}` ? `[ <a href="./pdfs/{{slides}}">slides</a> ]&nbsp;` : ``}
                ${`{{abstract}}` ? `[  <a  style="color:#268fd6;" class="collapsible">abstract&nbsp;</a> ] <div class="abstract rounded-corner"> <p> {{abstract}} </p></div>` : ``} 
                ${`{{cite}}` && `{{citelink}}` ? `Cited by <a href="{{citelink}}">{{cite}}</a>` : ``}
            </div>
        </div>
        <br />
        </li>`;
...
function yearSummary(arr, J, C, E, P, yr) {
    ...
    content =  `<div id="pubs_item"></div> <pub-sec 
        ...
        :cite="item.cite"
        :citelink="item.citelink"></pub-sec>`;   
        console.log(content)
        // .map((item) => {
        //     var count = -1;
        ...
        //     pubids[item['id']] = item['type'] + count;
            // return `<pub-sec :image="item.image"
            //                 :id="item.id"
            //                 :type="item.type"
            //                 :authors="item.authors"
            //                 :areas="item.areas"
            ...
            //                 :abstract="item.abstract"
            //                 :cite="item.cite"
            //                 :citelink="item.citelink"
            //                 :count="count"></pub-sec>`
    //         `<li class="li-pubs" id="${item['id']}">
    //     <div class="row">
    //     <div class="imgcolumn">
